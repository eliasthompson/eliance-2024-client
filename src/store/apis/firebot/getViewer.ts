...
export const { useLazyGetViewerQuery } = firebotApi.enhanceEndpoints({
  addTagTypes: ['VIEWER_DATA'],
}).injectEndpoints({
  endpoints: (build) => ({
    getViewer: build.query<FirebotApiGetViewerResponse | FirebotApiErrorResponse, FirebotApiGetViewerPayload>({
      query: ({ userId }) => ({
        method: 'GET',
        url: `/viewers/${userId}`,
      }),
